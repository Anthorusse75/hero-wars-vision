...
# Positions verticales relatives au centre de la ligne.
# Elles suivent les quatre valeurs visibles :
# puissance, dégâts, dégâts subis, soins.
METRIC_Y_OFFSETS = {
    "power": -0.345,
    "damage_dealt": -0.115,
    "damage_taken": 0.115,
...
# Géométrie des deux colonnes numériques autour du centre de l'écran.
VALUE_COLUMN_WIDTH_IN_FRAME_WIDTHS = 1.05
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Extrait les quatre valeurs numériques de chaque héros "
            "depuis les captures de statistiques Hero Wars."
        )
    )
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Remplace une extraction existante.",
    )
...
def main() -> int:
    ...
    return (
        0
        if len(crop_rows) == expected_crops
        else 1
    )
